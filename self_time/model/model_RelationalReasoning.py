...
class RelationalReasoning_InterIntra(torch.nn.Module):
    ...
    def train(self, tot_epochs, train_loader, opt):
        ...
        epoch_max = 0
        acc_max = 0
        print('[{}][{}]'.format(opt.model_name, opt.dataset_name))
        for epoch in range(tot_epochs):
            acc_epoch = 0
            acc_epoch_cls = 0
            loss_epoch = 0
            ...
            with tqdm(train_loader, unit="batch") as t_epoch:
                for i, (data, data_augmented0, data_augmented1, data_label, _) in enumerate(t_epoch):
                    ...
                    t_epoch.set_postfix(loss=loss_epoch / (i + 1), acc_epoch=acc_epoch / (i + 1),
                                        acc_epoch_cls=acc_epoch_cls / (i + 1), acc_max=acc_max, epoch_max=epoch_max)
