...
class DatasetV2(torch.utils.data.Dataset):
    ...
    def __init__(self, data, label, location, cluster,
                 aug_type: Union[list, str] = ('G2',), n_views=1):
        # assert all(tensors[0].size(0) == tensor.size(0) for tensor in tensors)
        from tsaug import TimeWarp, Crop, Quantize, Drift, Reverse, Convolve
...
def create_datasets_v2(data_file, norm=None, min_inv=0.4, augno=0, inv_state='none', aug_type='none', n_views=4):
    input_data = load_pickle(data_file)
    data_train = input_data["data_train"]
    label_train = input_data["label_train"]
    inv_train = input_data["inv_train"]
    ...
    cluster_file = np.load('/home/minh/PycharmProjects/prostate_cancer_classification/Samareh/TMI/debugs/clusters_TimeSeriesKmeans_50.npz', allow_pickle=True)
    core_len = cluster_file['core_len']
    _cluster_train = []
    cluster_centers_train = cluster_file['cluster_centers'].reshape((len(data_train), 50, -1))
    count = 0
    for cl in core_len:
        _cluster_train.append(cluster_file['y_list'][count: count+cl])
        count += cl
    ...
    for i in range(len(data_train)):
        if included_idx[i]:
            ...
            name_train.append(temp.reshape((data_train[i].shape[0], 8)))
            corelen_train.append(data_train[i].shape[0])
            cluster_train.append(_cluster_train[i])
            if inv_train[i] == 0:
                siginv_train.append(np.repeat(inv_train[i], data_train[i].shape[0]))
            else:
                siginv_train.append(np.repeat(inv_train[i], data_train[i].shape[0]))
    ...
    trn_ds = DatasetV2(signal_train, label_train, name_train,
                       aug_type=aug_type, n_views=n_views)  # ['magnitude_warp', 'time_warp'])
...
def create_datasets_test(data_file, state, norm='Min_Max', min_inv=0.4, augno=0, inv_state='normal', input_data=None,
    ...
    if input_data is None:
        input_data = hdf5storage.loadmat(data_file)
    p_name = "data_" + state
    data_test = input_data[p_name]  # [0]
    p_name = "label_" + state
    label_test = input_data[p_name]  # [0]
    p_name = "inv_" + state
    label_inv = input_data[p_name]  # [0]
    p_name = "corename_" + state
    CoreN_test = input_data[p_name]
    # print(CoreN_test)
    p_name = "PatientId_" + state
    patient_id_bk = input_data[p_name]  # [0]
    p_name = "inv_" + state
    involvement_bk = input_data[p_name]  # [0]
    p_name = "GS_" + state
    gs_bk = np.array(input_data[p_name])  # [0]
    ...
    if norm == 'Min_Max':
        ...
        for i in range(len(data_test)):
            if included_idx[i]:
                bags_test.append(norm01_rf(data_test[i], False))
    ...
    return tst_ds, corelen, label_inv, patient_id_bk, gs_bk
